...
class _ChatRoomState extends State<ChatRoom> {
  ...
  Future<String?> _uploadImage() async {
    ...
    Reference storageRef = FirebaseStorage.instance.ref().child('uploads/${widget.consultationId}/${widget.participants['userId']}/$fileName');
  ...
  Future<String?> _uploadFile() async {
    ...
    Reference storageRef = FirebaseStorage.instance
        .ref()
        .child('uploads/${widget.consultationId}/${widget.participants['userId']}/$fileName');
  ...
  Future<void> _generateConsultationRecord(String consultationId) async {
    try {
      ...
      await _firestore.collection('consultationRecords').add({
        ...
        'doctorId': widget.participants['doctorId'],
        'patientId': widget.participants['patientId'],
        'patientName': widget.participants['patientName'],
        'doctorName': widget.participants['doctorName'],
  ...
  void _navigateToSuccessPage() {
    final isUser = widget.participants['userId'] == _auth.currentUser?.uid;
    final isDoctor = widget.participants['doctorId'] == _auth.currentUser?.uid;
  ...
  Widget build(BuildContext context) {
    String? userId = widget.participants['userId'];
    String? doctorId = widget.participants['doctorId'];
    bool isDoctor = _auth.currentUser?.uid == doctorId;
    bool isUser = _auth.currentUser?.uid == userId;
    String doctorName = widget.participants['doctorName'] ?? 'Doctor';
    String userName = widget.participants['patientName'] ?? 'Patient';
                    ...
                    children: [
                      ...
                      Expanded(
                        child: ElevatedButton.icon(
                          ...
                          onPressed: () {
                            print(widget.participants['patientName']);
                            print(widget.participants);
                            if (widget.participants['patientName'] != null) {
                              Navigator.of(context).push(
                                MaterialPageRoute(
                                  builder: (context) => PatientMedicalInformationScreen(
                                   userName: widget.participants['patientName'],
